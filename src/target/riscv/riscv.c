...
static int vjtag_init(struct jtag_tap *tap)
{
...
static void vjtag_add_ir_scan(struct jtag_tap *tap, struct scan_field *ir_field, tap_state_t endstate)
{
	uint8_t t[4] = { 0 };
	struct scan_field field;

	uint32_t ir = buf_get_u32(ir_field->out_value, 0, ir_field->num_bits);

	if (!vjtag_ok) {
		if (vjtag_init(tap) != ERROR_OK) {
			jtag_add_ir_scan(tap, ir_field, endstate);
			vjtag = false;
			return;
		}
	}
	...
	buf_set_u32(t, 0, dr_length, (vjtag_node_address << m_width) | ir);
...
{
	if (vjtag)
		vjtag_add_ir_scan(tap, ir_field, endstate);
	else
		jtag_add_ir_scan(tap, ir_field, endstate);
}
